...
The Pointer input area is restricted to the _BlkSc_ page, not the screen as a whole, and the update rate is slower than in the ordinary case.
In addition the pointer does not update unless the _BlkSc_ page has focus.
...
However the synthesiser is more susceptible to interruption from other tasks, including _BlkSc_'s own editing operations.
